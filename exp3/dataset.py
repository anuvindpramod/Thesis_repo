...
transform_train=transforms.Compose([transforms.Resize((299,299)),transforms.RandomHorizontalFlip(p=0.5),transforms.ColorJitter(brightness=0.4,contrast=0.3,saturation=0.2),transforms.RandomAffine(degrees=25,translate=(0.1,0.1),shear=10),transforms.ToTensor(),transforms.Normalize(mean=[0.485, 0.456, 0.406], std=[0.229, 0.224, 0.225])])
...
class Mammogramddataset(Dataset):
        ...
        for patient_id in self.patient_ids:
            ...
            if all(v in views for v in ['LEFT_CC', 'LEFT_MLO', 'RIGHT_CC', 'RIGHT_MLO']):
                self.samples.append({
                    ...
                    'density':float(group['Density'].iloc[0])
                })
    ...
    def __getitem__(self, index):
        ...
        density_tensor = torch.tensor(sample['density'], dtype=torch.float32)
        
        return {
            'images': image_tensor,
            'age': torch.tensor(sample['age'], dtype=torch.float32),
            'density': torch.tensor(sample['density'], dtype=torch.float32)
            }
